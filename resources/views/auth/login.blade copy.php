                            ...
                            <form method="POST" action="{{ route('login') }}">
                                ...
                                <div class="input-group @error('password') has-error @enderror">
                                    ...
                                    <input id="password" name="password" required autocomplete="current-password"
                                        type="password" class="form-control" placeholder="{{ __('Password') }}">
                                    <div class="col-md-12">
                                        @error('password')
                                        <span class="help-block" role="alert">
                                            <strong>{{ $message }}</strong>
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <button type="submit" class="btn btn-sm col-12"
                                            style="color: #fff; background-color: #0D6EFD;">{{ __('Login') }}</button>
                                        <button type="button" class="btn btn-danger btn-sm col-12"
                                            onclick="window.location.href='{{ route('auth.google.redirect') }}'"> <i
                                                class="fa fa-google me-2"></i> <span style="padding-left: 20px">{{
                                                __('Login by Google') }} </span></button>
                                    </div>
                                </div>
                                <div class="text-center my-3">
                                    <span style="font-style: italic;">Belum Terdaftar ? <a
                                            href="{{ route('register') }}" style="color: #3e5879;">Klik
                                            disini untuk pendaftaran
                                            akun</a></span>
                                </div>
